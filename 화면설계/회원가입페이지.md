...
![필수값2](https://github.com/yyujjin/jjob-korea/blob/main/%ED%99%94%EB%A9%B4%EC%84%A4%EA%B3%84/images/giup_f2.jpg)

![필수값3](https://github.com/yyujjin/jjob-korea/blob/main/%ED%99%94%EB%A9%B4%EC%84%A4%EA%B3%84/images/giup_f3.jpg)

+ 필수값(*)을 입력하지 않으면 [필수정보입니다.]라는 알림창이 뜨며, 필수값이 하나라도 없다면 회원가입이 되지않는다.
  + 기업형태 : 선택박스로 몇가지의 선택지가 필요함 
  + 기업인증(사업자등록증면원 발급번호): 기입하지 않으면 [증명원 발급 번호를 재확인 해주세요.]라는 알림창
  + 회사주소: 선택박스를 사용하며, 시도와 시군구까지만.
# 회원가입 이후 화면
...
+ 1분 프로필은 직무, 근무지역, 기업형태 3가지 정도의 형태로 이 데이터를 채용정보와 결합하여 구직자가 채용정보를 카테고리별로 볼 수있게 한다
